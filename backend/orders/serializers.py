...
class OrderItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields= ["id","subtotal","price"]
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for item in items_data:
            ...
            cart_item = order.user.cart.items.filter(product_id=product_id).first()
            price = item.get("price") or (cart_item.product.price if cart_item else Product.objects.get(id=product_id).price)
